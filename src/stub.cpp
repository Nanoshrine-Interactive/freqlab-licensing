...
Status currentStatus() {
    ...
    // Precedence: LICENSED > EXPIRED > TAMPERED > GRACE > OVERDUE >
    // TRIAL > NOT_ACTIVATED > MODE (alias). Default is NoConfig (UI hidden).
...
#elif defined(FREQLAB_LICENSING_DEV_OVERDUE)
...
LicenseInfo current() {
    ...
    switch (info.status) {
        ...
        case Status::Trial:
            ...
            info.buyerName = std::string("Dev Tester");
            info.buyerEmail = std::string("dev@example.com");
            break;
        default:
            break;
    }
